...
namespace JetITeam\Base\Plugin\Theme\Block\Html\Header;
 ...
 * Config edit plugin.
 *
 * @package JetITeam\Base\Plugin\Theme\Block\Html\Header
 */
class LogoPlugin
{
    /**
    ...
    /**
     * Image or text logo template
     *
     * @return string
     *
     */
    public function aroundToHtml(Logo $subject, callable $proceed)
    {
        if($this->_helper->getLogoType() == 'text')
            $subject->setTemplate('JetITeam_Base::html/header/text_logo.phtml');
        return $proceed();
    }
